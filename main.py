...
def GetProcessedData():
    ...
    if "仅展示可评论的文章" in pin.pin["additional_features"]:
        df = df[df["commentable"] == True]  # 筛选允许评论的文章
    if "不展示付费文章" in pin.pin["additional_features"]:
        df = df[df["paid"] == False]  # 筛选不需要付费的文章
        ...
        for index, item in df.iterrows():
            ...
            if "开启 URL Scheme 跳转" in pin.pin["additional_features"]:  # 开启了 URL Scheme 跳转
                put_collapse(f"{item.title}", [
                    put_markdown(content, lstrip=True), 
...
def main_show():
    ...
    put_markdown("# 简书消零派辅助工具")
    
    put_markdown(f"""**消灭零评论，留下爱与光。**
    ...
    put_row([
        ...
        put_column([
            ...
            pin.put_checkbox("additional_features", label="高级选项", options=["开启 URL Scheme 跳转", "仅展示可评论的文章", 
                                            "不展示付费文章"]), 
